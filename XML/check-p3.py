...
if len(sys.argv) != 2:
    print
    sys.exit("Usage : $ python check-p3.py login_laboratorio")

if sys.argv[1] not in github_dict:
    print
    print "Usage: $ python check-p3.py login_laboratorio"
...
if len(student_file_list) != 6:
    error = 1
    print "Error: solamente hay que subir al repositorio los ficheros indicados en las guion de practicas, que son en total 6:"
...
if not error:
    ...
    print
    os.system('pep8 --repeat --show-source --statistics /tmp/' + aleatorio + '/smallsmilhandler.py /tmp/' + aleatorio + '/karaoke.py')
print
